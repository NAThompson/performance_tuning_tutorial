...
- No noticeable slowdown under `perf`
...
Small optimizations are really important, and really hard to measure reliably.
...
Compile the example with and without `-ffast-math`, with and without AVX-512 (must use `clang` or `icc`).

Are instructions or [uops](https://uops.info/) a better proxy for runtime?

---

## Exercise

Parallelize the dot product using a framework of your preference (OpenMP, C++ threads, TBB).
