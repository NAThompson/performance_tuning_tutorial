...
Navigate to function of interest and hit 'a'
...
```
$ perf stat -e instructions,cycles -r 5 ./dot 100000000
 Performance counter stats for './dot 100000000' (5 runs):

     2,301,763,921      instructions:u            #    1.20  insn per cycle           ( +-  0.00% )
     1,923,387,208      cycles:u                                                      ( +-  2.23% )

            1.6901 +- 0.0198 seconds time elapsed  ( +-  1.17% )
```

Instruction count is reproducible, but time and cycles are not. Use instruction count as an imperfect metric for small optimizations.
